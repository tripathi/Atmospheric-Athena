...
static Real GM,rp,rin,rout,rreset,rho0,csq,Rsoft;
...
void problem(DomainS *pDomain)
{
  ...
  fprintf(stderr,"Spacing between reset rad (%f) and edges is %f cells \n", rreset, (rp-rreset)/pGrid->dx1);
  ...
  rho0 = rhop / exp(GM / Gamma/ csq * (1/rp - 1/rin));
  rho_out = rhop;
  fprintf(stderr,"Central density is %e. Ext density is %e \n", rho0, rho_out);
...
/* setup uniform ambient medium with spherical over-pressured region */
  ...
  for (k=ks; k<=ke; k++) {
	...
	if (rad > rout) {
	  ...
	  pGrid->U[k][j][i].E = rho_out * csq / Gamma_1;
	} else {
	  myrho = rho0 * exp(GM / Gamma/ csq * (1/rad - 1/rin));
	  pGrid->U[k][j][i].d  = MIN(rho0,myrho);
	  pGrid->U[k][j][i].E  = pGrid->U[k][j][i].d * csq / Gamma_1;
	}
      }
    }
  }
