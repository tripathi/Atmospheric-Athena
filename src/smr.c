...
#ifndef BAROTROPIC
            pG->U[k][j][i].E  = *(pRcv++);
	    if (pG->U[k][j][i].E < 0) {ath_error("I have a negative energy at line 196 \n");}
#endif /* BAROTROPIC */
#if (NSCALARS > 0)
...
/* 2D/3D problem: Add conservative average in x2 */
      ...
      if (pG->Nx[1] > 1) {
        ...
        fact = 0.25;
        nCons = (jpe-jps+1)*(ipe-ips+1)*(nvarion)/4;
      }
...
/* 3D problem: Add conservative average in x3 */
      ...
      if (pG->Nx[2] > 1) {
        ...
        fact = 0.125;
        nCons = (kpe-kps+1)*(jpe-jps+1)*(ipe-ips+1)*(nvarion)/8;
      }
